...
class MyEnv(gym.Env):
    ...
    def __init__(
        ...
        rewards : list of float, optional
            Rewards assigned to each position, by default [-1, 0, 0, 0, 5].
    ...
    def step(
        ...
        if action == 0:
            self.position = max(0, min(self.states[-1], self.position - 1))
        else:
            self.position = max(0, min(self.states[-1], self.position + 1))
