...
class REINFORCEAgent(AbstractAgent):
    ...
    def update_agent(
        ...
        # Unbiased on PyTorch's Tensor.std -> see forum
        norm_returns = (returns_t - returns_t.mean()) / (returns_t.std(False) + 1e-8)
    ...
    def evaluate(
        ...
        for _ in range(num_episodes):
            state, temp = eval_env.reset()
            done = False
            episode_return = 0.0

            while not done:
                action, temp = self.predict_action(state, evaluate=True)
                state, reward, term, trunc, temp = eval_env.step(action)
                done = term or trunc
