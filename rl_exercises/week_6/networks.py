...
class ValueNetwork(nn.Module):  # critic network
    ...
    def __init__(self, state_space: gym.spaces.Box, hidden_size: int = 128):
        ...
        self.net = nn.Sequential(
            nn.Linear(self.state_dim, hidden_size), nn.ReLU(), nn.Linear(hidden_size, 1)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        value_network_output = self.net(x).squeeze()

        return value_network_output  # TODO: replace with your value network output
